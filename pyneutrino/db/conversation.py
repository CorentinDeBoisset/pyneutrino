...
class Conversation(db.Model):  # type: ignore[name-defined]
    ...
    creator: Mapped[str] = mapped_column(Uuid, ForeignKey("user_account.id"), nullable=False)

    # We either have an account liked to the receiver, or an anonymous receiver where we only save the public key
    receiver: Mapped[str] = mapped_column(Uuid, ForeignKey("user_account.id"))
    reveiver_public_key: Mapped[str] = mapped_column(String)
