...
class SentMessage(db.Model):  # type: ignore[name-defined]
    ...
    conversation: Mapped[str] = mapped_column(Uuid, ForeignKey("conversation.id"), nullable=False)
    sender: Mapped[str] = mapped_column(Uuid, ForeignKey("user_account.id"), nullable=False)
    creation_date: Mapped[datetime] = mapped_column(DateTime)
    message: Mapped[int] = mapped_column(String)
